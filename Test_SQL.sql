...
create or replace function stack.select_count_pok_by_service (text, text)
returns table (number int, service int, count int)
language sql
as 'select a.number, c.service, count(mp.value) from stack.meter_pok as mp
	join stack.counters as c on c.row_id = mp.counter_id
	join stack.accounts as a on a.row_id = c.acc_id
	where c.service = cast($1 as int) and mp.month = cast($2 as date)
...
returns table (number int, name text, value int)
language sql
as 'select a.number, c.name, sum(mp.value) from stack.meter_pok as mp
	join stack.counters as c on c.row_id = mp.counter_id
	join stack.accounts as a on a.row_id = c.acc_id
	where mp.month = cast($2 as date) and
		((parent_id in									-- лицевой счет привязан к дому
			(select row_id from stack.accounts 			-- row_id записи типа "дом", номер дома - аргумент 1
				where type = 1 and number = $1)  				
			and type = 3)
		or 	parent_id in								-- лицевой счет привязан к квартире или другому лицевому счету
				(select row_id from stack.accounts
				where 	parent_id in
						(select row_id from stack.accounts 
							where type = 1 and number = $1)
		))
...
create or replace function stack.select_last_pok_by_acc (int)
returns table (number int, service int, date date, tarif int, value int)
language sql
as 'with subq as (select a.number, c.service, max(mp.date) as date 
					from stack.meter_pok as mp
						join stack.counters as c on c.row_id = mp.counter_id
						join stack.accounts as a on a.row_id = c.acc_id
						where a.number = $1
						group by a.number, c.service)
	select subq.number, subq.service, subq.date, mp2.tarif, mp2.value from subq 
		join stack.accounts as a2 on a2.number = subq.number
		join stack.meter_pok as mp2 on mp2.acc_id = a2.row_id
		join stack.counters as c2 on c2.row_id = mp2.counter_id
		where subq.date = mp2.date and subq.service = c2.service'
;
...
select * from stack.select_last_pok_by_acc(111);
